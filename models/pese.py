...
from utils.inc_net import PeseNet
...
class Learner(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self._network = PeseNet(args, True)
    ...
    def solve_similarity(self):       # 没使用语义映射
    ...
    def solve_sim_reset(self):       # 使用语义映射
        for task_id in range(self._cur_task):
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if not self.args.get('perb', False):
                    output = self._network(inputs, test=False)
                    logits = output["logits"]
                    loss = F.cross_entropy(logits, aux_targets)
                else:
                    perturb_output = []
                    perb = self.args['perb']  # 设置成超参数
                    for i in range(5):
                        model = deepcopy(self._network)
                        with torch.no_grad():
                            # print([name for name, param in self._network.named_parameters()])
                            for name, param in model.named_parameters():
                                if ('bias' in name) or ('bn' in name) or (not param.requires_grad):
                                    # 不对偏置和BatchNorm的参数添加噪声
                                    continue
                                std = perb * param.data.std()
                                if std > 0.0:
                                    noise = torch.normal(0, std, size=param.size()).to(self._device)
                                    param.data.add_(noise)
                            perturb_output.append(model(inputs, test=False)["logits"])
                    # print()
                    # print(torch.stack(perturb_output).shape)
                    tmp = torch.stack(perturb_output)
                    # norms = torch.norm(tmp, dim=(0, 2), p='nuc')
                    norms = torch.linalg.norm(tmp, dim=(0, 2), ord='nuc')
                    norms = (1.0 / norms) * torch.max(norms)
                    weights = norms
                
                
                    output = self._network(inputs, test=False)
                    logits = output["logits"]
                    loss = self._my_CE_loss(logits, aux_targets, weights)
                    # loss = F.cross_entropy(logits, aux_targets)
    ...
    # 自定义的交叉熵损失函数
    def _my_CE_loss(self, logits, labels, weights):
        # 计算softmax
        softmax = torch.exp(logits) / torch.exp(logits).sum(dim=1, keepdim=True)

        # 获取正确类别的概率
        correct_class_probabilities = softmax[range(len(labels)), labels]  #! 用两个索引来选择 softmax 张量中的元素。第一个索引 range(len(labels)) 选择行，第二个索引 labels 选择列。具体来说，对于每个样本 i，我们从 softmax 的第 i 行中选择第 labels[i] 列的元素，即第 i 个样本正确类别的概率。

        # 计算交叉熵损失
        loss = -torch.log(correct_class_probabilities)

        # 计算加权损失
        # weighted_loss = torch.sum(loss * weights) / torch.sum(weights)
        weighted_loss = torch.sum(loss * weights) / labels.shape[0]
