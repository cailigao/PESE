...
class BaseLearner(object):
    def __init__(self, args):
        ...
        self.topk = args["top_k"] if args.get("top_k", None) is not None else 1
    ...
    def tsne(self,showcenters=False,Normalize=False):
        ...
        plt.savefig(str(self.args['model_name'])+str(tot_classes)+'tsne.svg', format='svg', bbox_inches='tight', dpi=600)
        plt.close()
    ...
    def plot_matrix(self, y_true, y_pred, labels_name, title=None, thresh=0.8, axis_labels=None):
        ...
        # 绘制坐标
        num_local = np.array(range(0, len(labels_name)))
        num_local_labels = np.array(range(1, len(labels_name)+1))
        # num_local = np.arange(10, 101, 10)
        if axis_labels is None:
            axis_labels = labels_name
        pl.xticks(num_local, labels=num_local_labels, rotation=45)  # 将标签印在x轴坐标上， 并倾斜45度
        pl.yticks(num_local, labels=num_local_labels)  # 将标签印在y轴坐标上
        pl.ylabel('True class', labelpad=-3)
        pl.xlabel('Predicted class', labelpad=-3)

        pl.savefig(f'{self.args["model_name"]}_matrix.svg', format='svg', bbox_inches='tight', dpi=600)
        pl.show()

    def _evaluate(self, y_pred, y_true):
        ret = {}
        grouped = accuracy(y_pred.T[0], y_true, self._known_classes, self.args["init_cls"], self.args["increment"])
        if np.max(y_true) + 1 == self.data_manager.nb_classes:  # 绘制最后一个阶段的混淆矩阵
            labels_name = [i for i in range(np.max(y_true) + 1)]
            self.plot_matrix(y_true, y_pred.T[0], labels_name=labels_name)
        ret["grouped"] = grouped
