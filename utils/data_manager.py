...
from utils.data import CIL_Forgery
import random
...
class DataManager(object):
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        if shuffle:
            random.seed(seed)
            step = self.args["init_cls"]
            sublists = [order[i:i+step] for i in range(0, len(order), step)]
            random.shuffle(sublists)
            order = [item for sublist in sublists for item in sublist]
