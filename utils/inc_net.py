...
from backbone.linears import PeseCosineLinear
import timm
...
def get_backbone(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        if args["model_name"] == "pese" :
            from backbone import vit_pese
            ...
            if name == "swin_base_patch4_window7_224_pese":
                model = vit_pese.swin_base_patch4_window7_224_pese(num_classes=0,
                    drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=1024
            elif name == "vit_base_patch16_224_pese":
                model = vit_pese.vit_base_patch16_224_pese(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            elif name == "vit_base_patch16_224_in21k_pese":
                model = vit_pese.vit_base_patch16_224_in21k_pese(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            else:
...
class PeseNet(BaseNet):
    ...
    def generate_fc(self, in_dim, out_dim):
        fc = PeseCosineLinear(in_dim, out_dim)
        return fc
